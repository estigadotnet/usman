 ...
 */
class orders_by_product extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		// CategoryName
		$this->CategoryName = new DbField('orders_by_product', 'orders_by_product', 'x_CategoryName', 'CategoryName', '`CategoryName`', '`CategoryName`', 200, 15, -1, FALSE, '`CategoryName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->CategoryName->Sortable = TRUE; // Allow sort
		$this->fields['CategoryName'] = &$this->CategoryName;

		// ProductName
		$this->ProductName = new DbField('orders_by_product', 'orders_by_product', 'x_ProductName', 'ProductName', '`ProductName`', '`ProductName`', 200, 40, -1, FALSE, '`ProductName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ProductName->Sortable = TRUE; // Allow sort
		$this->fields['ProductName'] = &$this->ProductName;

		// CompanyName
		$this->CompanyName = new DbField('orders_by_product', 'orders_by_product', 'x_CompanyName', 'CompanyName', '`CompanyName`', '`CompanyName`', 200, 40, -1, FALSE, '`CompanyName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->CompanyName->Sortable = TRUE; // Allow sort
		$this->fields['CompanyName'] = &$this->CompanyName;

		// OrderDate
		$this->OrderDate = new DbField('orders_by_product', 'orders_by_product', 'x_OrderDate', 'OrderDate', '`OrderDate`', CastDateFieldForLike("`OrderDate`", 0, "DB"), 135, 19, 0, FALSE, '`OrderDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Amount
		$this->Amount = new DbField('orders_by_product', 'orders_by_product', 'x_Amount', 'Amount', '`Amount`', '`Amount`', 5, 23, -1, FALSE, '`Amount`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "orders_by_productlist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "orders_by_productview.php")
			return $Language->phrase("View");
		elseif ($pageName == "orders_by_productedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "orders_by_productadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("orders_by_productview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("orders_by_productview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "orders_by_productadd.php?" . $this->getUrlParm($parm);
		else
			$url = "orders_by_productadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("orders_by_productedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("orders_by_productadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("orders_by_productdelete.php", $this->getUrlParm());
	}
